/**
 * @file loader.c
 * @brief Lynceus Control Plane - Parallel Monster Extractor (v2.0-Hybrid).
 * 
 * @details 
 * Combined architecture: Massively Parallel Shared-Nothing Performance (v2.0)
 * integrated with the 400+ Feature Monster analytical engine.
 */
...
struct flow_state {
    ...
    uint64_t active_start, idle_start;
    int active;
};
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
    struct worker_t *w = ctx; const struct packet_event_t *e = data;
    ...
    if (!w->flow_table[idx].active) {
        ...
        w_init(&w->flow_table[idx].active_s); w_init(&w->flow_table[idx].idle_s);
    ...
    /* 400-Feature Serialization (High-Density) */
    w->s_off += snprintf(w->s_buf + w->s_off, 8192, "%s-%s-%u-%u-%u,%.6f,%s,%u,%s,%u,%u,%.6f,%lu,%lu,%lu,%lu,%lu,%lu,", sip, dip, ntohs(e->key.src_port), ntohs(e->key.dst_port), e->key.protocol, ts, sip, ntohs(e->key.src_port), dip, ntohs(e->key.dst_port), e->key.protocol, duration, s->t_pay.n, s->f_pay.n, s->b_pay.n, s->f_bytes + s->b_bytes, s->f_bytes, s->b_bytes);
    ...
    FMT_W(s->active_s); FMT_W(s->idle_s);
    ...
    if (w->s_off > IO_BUFFER_SIZE - 8192) { fwrite(w->s_buf, 1, w->s_off, w->out_f); w->s_off = 0; }
...
void *worker_fn(void *arg) {
    ...
    const char *metrics[] = {"pay", "fwd_pay", "bwd_pay", "hdr", "fwd_hdr", "bwd_hdr", "iat", "fwd_iat", "bwd_iat", "active", "idle"};
    for(int i=0; i<11; i++) fprintf(w->out_f, "%s_max,%s_min,%s_mean,%s_std,%s_var,%s_skew,%s_kurt,", metrics[i], metrics[i], metrics[i], metrics[i], metrics[i], metrics[i], metrics[i]);
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "🚀 [Lynceus Core] %d Workers (400+ Feature Engine v2.0-Parallel)\n", num_workers);
