...
def get_tcpreplay_stats(stderr_text):
    """Extracts the real packet count from tcpreplay stderr."""
...
def process_pcap_dir(pcap_dir, category_tag):
    ...
    for p in pcaps:
        ...
        # We use a combined command but don't redirect stderr to /dev/null
        cmd = f"sudo tcpdump -r {p} -w - 2>/dev/null | sudo tcpreplay -i lo -t -"
        try:
            # We want to see the output and also capture it
            # Using Popen to stream the output to the console and capture it simultaneously
            process = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE, text=True)
            _, stderr_content = process.communicate()
            
            # Print tcpreplay stats to console for user visibility
