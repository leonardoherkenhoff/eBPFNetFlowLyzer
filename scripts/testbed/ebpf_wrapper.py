...
def process_pcap_dir(pcap_dir, category_tag):
    ...
    for p in pcaps:
        ...
        # We combine stdout and stderr into one pipe to ensure we capture all stats
        cmd = f"sudo tcpdump -r {p} -w - 2>/dev/null | sudo tcpreplay -i lo -t - 2>&1"
        try:
            # Capture output from the combined stream
            res = subprocess.run(cmd, shell=True, capture_output=True, text=True, check=True)
            output_content = res.stdout
            
            # Print output to console for user visibility
